  ...
  list: [
    {
      ...
      where: '뉴소프트',
      descriptions: [
        { content: 'WebSocket 게이트웨이와 Redis Pub/Sub로 시세·체결 이벤트를 브로드캐스트, 폴링 제거로 실시간 처리 부하 분산' },
        { content: '주문/포지션 API에 idempotency 키와 비동기 확인 큐 적용, 재시도에도 중복 체결 방지' },
    ...
    },
    {
      ...
      where: '뉴소프트',
      descriptions: [
        { content: '신규 차종/옵션/프로모션을 규칙 엔진으로 모델링, 계산 경로를 캐시해 반복 견적 부하 흡수' },
        { content: '외부 견적/재고 API 호출을 비동기화하고 서킷브레이커·버스트 제한으로 스파이크 트래픽 보호' },
